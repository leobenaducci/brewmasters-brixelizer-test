	...
	, m_PlayerController(m_Camera, m_Input)
{
	...
	m_GBufferShader  = std::make_unique<Shader>(device, L"D:\\projects\\brixelizer-test\\Engine\\Resources\\Shaders\\GBuffer.hlsl");
	m_LightingShader = std::make_unique<LightingShader>(device, L"D:\\projects\\brixelizer-test\\Engine\\Resources\\Shaders\\Lighting.hlsl");

	m_BrixelizerContext = std::make_unique<Brixelizer::BrixelizerContext>(device, app.GetDX().GetCommandQueue());
	...
	DirectX::XMStoreFloat4x4(&m_PrevProjection, m_Camera.GetProjectionMatrix());
}
...
void AppLayer::OnRender() {
	...
	// Light pass.
	{
		...
		m_Model.Draw(cmdList, srvHeap, device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
	}

	m_PrevView = view;
	m_PrevProjection = proj;
}
...
void AppLayer::UpdateDiffuseGISRV(ID3D12Device* device) {
	...
	device->CreateShaderResourceView(m_BrixelizerGIContext->GetDiffuseGI(), &srvDesc, handle);
}