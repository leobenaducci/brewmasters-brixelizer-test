...
public:
    ...
    DX12Manager(HWND const windowHandle) {
        std::cout << "[Engine] Iniciando DX12 Pipeline con Double Buffering..." << std::endl;
        try {
            ...
            std::cout << "[Engine] DX12 Pipeline listo." << std::endl;
        }
...
private:
    void InitPipeline(HWND const hwnd) {
        ...
        for (UINT i = 0; m_Factory->EnumAdapters1(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i) {
            ...
            // Omitir el software renderer (WARP)
            if (desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) {
                continue;
            } 

            // Seleccionar la que tenga más memoria de video dedicada
        ...
        // Fallback a WARP si no se encontró hardware compatible
        if (FAILED(hr)) {
