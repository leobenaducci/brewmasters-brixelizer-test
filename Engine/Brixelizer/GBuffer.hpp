...
// ---------------------------------------------------------------------------
// G-Buffer mínimo para Brixelizer GI
//
// RTs:
...
//
// Dummies (valor constante, nunca se escriben desde shaders):
//   roughness     DXGI_FORMAT_R8_UNORM         (= 0, sin specular)
//   motionVectors DXGI_FORMAT_R16G16_FLOAT     (= 0, sin TAA)
//
// History (copias CPU→GPU del frame anterior):
...
public:
    ...
    void Init(
        ...
        ID3D12DescriptorHeap* rtvHeap, uint32_t rtvOffset,   // 1 RTV slot
        ID3D12DescriptorHeap* dsvHeap, uint32_t dsvOffset    // 1 DSV slot
    ) {
        ...
        {
            // Textura 1x1 en estado SRV — valor 0 siempre
            D3D12_HEAP_PROPERTIES heap { .Type = D3D12_HEAP_TYPE_DEFAULT };
            D3D12_RESOURCE_DESC   desc {
                .Dimension        = D3D12_RESOURCE_DIMENSION_TEXTURE2D,
                .Width            = 1,
        ...
        // Depth (formato typeless para poder leerlo como SRV R32_FLOAT)
        {
            D3D12_HEAP_PROPERTIES heap { .Type = D3D12_HEAP_TYPE_DEFAULT };
            D3D12_RESOURCE_DESC desc {
                .Dimension        = D3D12_RESOURCE_DIMENSION_TEXTURE2D,
    ...
    void CopyToHistory(ID3D12GraphicsCommandList* cmd) {
        ...
        // Depth se copia como R32_TYPELESS → R32_FLOAT en history
        {
