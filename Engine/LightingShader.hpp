...
//
// Static sampler s0 — trilinear wrap.
...
public:
	LightingShader(ID3D12Device* device, const std::wstring& path) {
		...
		if (FAILED(hr)) {
			if (err) {
				std::cerr << "LightingShader RootSig error: " << (char*)err->GetBufferPointer() << "\n";
			}

			DX_THROW(hr);
		}

		DX_THROW(device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&m_RootSignature)));
		...
		hr = D3DCompileFromFile(path.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_0", flags, 0, &vsBlob, &errBlob);
		if (FAILED(hr)) {
			if (errBlob) {
				std::cerr << "LightingShader VS: " << (char*)errBlob->GetBufferPointer() << "\n";
			}
		...
		if (FAILED(hr)) {
			if (errBlob) {
				std::cerr << "LightingShader PS: " << (char*)errBlob->GetBufferPointer() << "\n";
			}

			DX_THROW(hr);
		}

		// Input layout (see Shader.hpp)
		...
		if (FAILED(hr)) {
			std::cerr << "[ERROR] LightingShader PSO failed (0x" << std::hex << hr << ")\n";
			DX_THROW(hr);
		}
	}
